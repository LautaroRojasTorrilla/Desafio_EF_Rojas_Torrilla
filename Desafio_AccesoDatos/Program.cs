    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                int idAEliminar = 2;
                bool eliminado = ProductoService.EliminarProductoPorID(idAEliminar);
                ...
                {
                    Console.WriteLine($"No se encontró un producto con ID {idAEliminar} para eliminar.");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error general en el programa: {ex.Message}");
            }
            finally
            {
